...
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    zim.Board = function(size, cols, rows, backgroundColor, rollBackgroundColor, borderColor, borderWidth, icon, isometric, indicatorColor, indicatorBorderColor, indicatorBorderWidth, indicatorSize, indicatorType, arrows, arrowColor, arrowRollColor, swipe, info, labels, color, scaleMin, scaleMax, swipe, buffer) {
        ...
        function outHandler(e) {
            that.timeout = zim.timeout(50, function () {
        ...
        this.move = function(obj, col, row, time, type) {
            ...
            if (obj.boardCol!=newCol || obj.boardRow!=newRow) obj.animate({
        ...
        this.followPath = function(obj, path, time, animation, buffer) {
            ...
            if (zot(time)) time = 600;
            if (zot(animation) || animate===true) animation = 300;
    ...
    zim.Orb = function(radius, color, color2, accentColor, accentColor2, flat, alpha, time, delay) {
        ...
        if (flat) {
            new zim.Circle(radius, color, accentColor)
                ...
                .sca(1,1)
                .addTo(this)
                .animate({obj:{alpha:alpha}, time:1000, loop:true, rewind:true});
...
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    zim.Timer = function(time, step, colon, down, isometric, startPaused, size, font, color, backgroundColor, borderColor, borderWidth, align, valign, fontOptions, width, height, decimals) {
        ...
        this.super_constructor(colon?zim.decimals(time/60,2,2,1,null,true):time, size, font, color, null, null, null, align, valign, null, null, fontOptions, new Rectangle(width, height, backgroundColor, borderColor, borderWidth).centerReg({add:false}));
        this.type = "Timer";
        var that = this;
        that.totalTime = time;
...
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    zim.Scorer = function(score, isometric, size, font, color, backgroundColor, borderColor, borderWidth, align, valign, fontOptions, width, height) {
        ...
        this.super_constructor(score, size, font, color, null, null, null, align, valign, null, null, fontOptions, new Rectangle(width, height, backgroundColor, borderColor, borderWidth).centerReg({add:false}));
        this.type = "Scorer";
        var that = this;
